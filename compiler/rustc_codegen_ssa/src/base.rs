...
fn analyze_def_use_info<'tcx>(tcx: TyCtxt<'tcx>) {
    ...
    // We're in a binary leaf crate, we can analyze:
    // 1) each crate's unused definitions from all the recorded statistics about defs and uses.
...
fn dump_def_use_info<'tcx>(tcx: TyCtxt<'tcx>, codegen_units: &[&CodegenUnit<'tcx>]) {
    ...
    for index in 0..defs.def_index_count() {
        ...
        match tcx.def_kind(def_id) {
            DefKind::Fn => {}
            DefKind::AssocFn if tcx.impl_of_method(def_id).is_some() => {}
            // DefKind::Closure => {}
            _ => continue,
        }
        ...
        for (item, _data) in cgu.items() {
            match item {
                MonoItem::Fn(instance) => {
                    ...
                    // FIXME: - Can we actually filter these items in this manner?
                    // - We're sometimes asked to mono closures, but how are they actually codegened?
                    match tcx.def_kind(def_id) {
                        DefKind::Fn => {}
                        DefKind::AssocFn if tcx.impl_of_method(def_id).is_some() => {}
                        // DefKind::Closure => {}
