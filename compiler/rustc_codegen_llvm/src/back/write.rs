...
fn get_params(fnc: &Value) -> Vec<&Value> {
    // SAFETY: Assumes that Value is a function.
    unsafe {
...
pub(crate) fn enzyme_ad(
    ...
    unsafe {
        ...
        //
        // FIXME(ZuseZ4): enable this, but with the right position of the arg
...
pub(crate) unsafe fn differentiate(
    ...
    for item in higher_order_items {
        let res = enzyme_ad(llmod, llcx, &diag_handler.handle(), item, logic_ref, ad);
        assert!(res.is_ok());
    }

    // Remove these attributes after Enzyme is done,
    // since we have only added them to prevent a specific LLVM optimization around enums.
    // TODO(ZuseZ4, wsmoses): This probably wants some further discussions.
    unsafe {
