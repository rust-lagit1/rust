    ...
    'tcx: 'b,
{
    ...
    fn build_async_drop(
        ...
        let (fut_ty, drop_fn_def_id, trait_args) =
            if call_destructor_only {
                ...
                let (drop_trait, trait_args) =
                    match tcx.codegen_select_candidate((ty::ParamEnv::reveal_all(), trait_ref)) {
                        Ok(traits::ImplSource::UserDefined(
                            traits::ImplSourceUserDefinedData { impl_def_id, args, .. },
                        )) => (*impl_def_id, *args),
                        impl_source => {
    ...
    fn build_drop(&mut self, bb: BasicBlock) {
        ...
        if self.tcx().features().async_drop
            ...
            && drop_ty.needs_async_drop(self.tcx(), self.elaborator.param_env())
        {
    ...
    fn open_drop_for_box_contents(
        ...
        self.drop_subpath(interior, interior_path, succ, unwind, dropline)
    }
    ...
    fn destructor_call_block(
        ...
        if self.tcx().features().async_drop
            ...
            && self.elaborator.allow_async_drops()
            && !unwind.is_cleanup()
            && ty.is_async_drop(self.tcx(), self.elaborator.param_env())
        {
    ...
    fn drop_loop(
        ...
        if self.tcx().features().async_drop
            ...
            && self.elaborator.allow_async_drops()
            && !unwind.is_cleanup()
            && ety.needs_async_drop(self.tcx(), self.elaborator.param_env())
        {
            self.build_async_drop(
                place,
                ety,
    ...
    fn drop_loop_trio(&mut self, ety: Ty<'tcx>) -> BasicBlock {
        debug!("drop_loop_trio({:?})", ety);
        let tcx = self.tcx();
    ...
    fn open_drop(&mut self) -> BasicBlock {
        ...
        match ty.kind() {
            ...
            ty::Slice(ety) => self.drop_loop_trio(*ety),
