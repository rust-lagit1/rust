...
impl<'a, 'tcx> Annotator<'a, 'tcx> {
        ...
        F: FnOnce(&mut Self),
    {
        ...
        let mut const_spans = None;

        let const_stab = const_stab.map(|(const_stab, const_stab_spans)| {
            let const_stab: &_ = self.tcx.arena.alloc(const_stab);
            self.index.const_stab_map.insert(def_id, const_stab);
            const_spans = Some(const_stab_spans);
            const_stab
        });
        ...
        if let (Some(const_spans), Some(fn_sig)) = (&const_spans, fn_sig)
        ...
        let stab = stab.map(|(stab, stab_spans)| {
            ...
            // Check if any already-stable features are on unstable or const-unstable attrs.
            for (level, span) in stab_spans
                .iter()
                .chain(const_spans.iter().flat_map(|level_spans| level_spans.iter()))
            {
                if ACCEPTED_FEATURES.iter().any(|f| level.has_unstable_feature(f.name)) {
